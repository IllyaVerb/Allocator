 ...
 */
void*
avl_walk(struct avl_tree* tree, void* oldnode, int left)
{
	...
	/*
	 * if we go NEXT and it root of tree, return NULL
	 */
	if (oldnode == tree->avl_root && left == AVL_AFTER)
		return (NULL);
...
avl_add(avl_tree_t* tree, void* new_node)
{
	...
	if (avl_find(tree, new_node, &where) != NULL) {
		push_front_tree_list(tree, 
			(tree_data*)AVL_NODE2DATA(AVL_INDEX2NODE(where), tree->avl_offset), new_node);
	}
...
avl_remove(avl_tree_t* tree, void* data) {
	...
	avl_index_t where;
	if (avl_find(tree, data, &where) != NULL) {
		tree_data* head = (tree_data*)AVL_NODE2DATA(AVL_INDEX2NODE(where), tree->avl_offset);
		if (head->next != NULL) {
			return remove_any_tree_list(tree, head, data);
		}
	}

	return avl_remove_in_tree(tree, data);
}
 ...
 */
void*
avl_remove_in_tree(avl_tree_t* tree, void* data)
{
	avl_node_t* delete;
	avl_node_t* parent;
	avl_node_t* node;
...
static void avl_traverse_impl(avl_tree_t *tree, avl_node_t *node, void (*func)(size_t, void*)) {
	if (node == NULL)
		return;
	if (node->avl_child[0] != NULL)
		avl_traverse_impl(tree, node->avl_child[0], func);
	func(0, AVL_NODE2DATA(node, tree->avl_offset));

	tree_data* tmp_node = AVL_NODE2DATA(node, tree->avl_offset);
	...
	if (node->avl_child[1] != NULL)
		avl_traverse_impl(tree, node->avl_child[1], func);
}

void avl_traverse(struct avl_tree *tree, void (*func)(size_t, void*)) {
	avl_traverse_impl(tree, tree->avl_root, func);
}

static void push_front_tree_list(avl_tree_t *tree, tree_data* list_head, void* new_node) {
	((tree_data*)new_node)->next = list_head;
	list_head->prev = new_node;

	avl_remove_in_tree(tree, list_head);
	avl_add(tree, new_node);
}

static void* pop_front_tree_list(avl_tree_t *tree, tree_data* list_head) {
	void* new_head = list_head->next;

	list_head->next->prev = NULL;
	list_head->next = NULL;

	avl_remove_in_tree(tree, list_head);
	avl_add(tree, new_head);

	return list_head;
}

static void* remove_any_tree_list(avl_tree_t *tree, tree_data* list_head, void* node) {
	if (list_head == node || node == NULL)
		return pop_front_tree_list(tree, list_head);

	tree_data* tmp_node = node;
...
void* best_fit(struct avl_tree *tree, size_t search_size) {
	void* less_data = avl_first(tree);

	while (1) {
		if (less_data == NULL)
			return NULL;
		if (search_size <= ((tree_data*)less_data)->size)
			return less_data;
		less_data = AVL_NEXT(tree, less_data);
	}
}